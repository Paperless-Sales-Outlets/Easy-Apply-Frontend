...
export default function AgreementStep({ isActive }) {
  ...
  return (
    <div>
      <h3 style={{ color: 'var(--slt-blue)', marginBottom: '1.5rem' }}>{t('wizards.locationChange.agreement.heading')}</h3>
      ...
      <div className="card" style={{ padding: '1.5rem', backgroundColor: 'var(--surface-color)', border: '1px solid var(--border-color)', marginBottom: '1.5rem' }}>
        ...
        <label className="checkbox-label" style={{ fontWeight: '500', color: 'var(--text-primary)' }}>
          <input type="checkbox" className="checkbox-input" required={isActive} /> {t('wizards.locationChange.agreement.agreeLabel')}
        </label>
      </div>
