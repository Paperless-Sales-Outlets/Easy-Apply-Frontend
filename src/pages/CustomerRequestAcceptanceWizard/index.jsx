...
export default function CustomerRequestAcceptanceWizard() {
  ...
  const handleSubmit = (e) => {
    ...
    alert("Customer Request Acceptance application submitted successfully!");
    navigate('/');
  };

  return (
    <div className="card" style={{ padding: '3rem', width: '100%', margin: '0 auto' }}>
      <h2 style={{ marginBottom: '1.5rem' }}>{t('wizards.customerRequestAcceptance.title')}</h2>
      <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>{t('wizards.customerRequestAcceptance.subtitle')}</p>
        ...
        {[1, 2].map(step => (
          <div key={step} className="wizard-step" style={{ position: "relative", zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem", flex: 1 }}>
            ...
            <span style={{ fontSize: '0.8rem', color: step <= currentStep ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
              {step === 1 ? t('wizards.customerRequestAcceptance.steps.s1') : t('wizards.customerRequestAcceptance.steps.s2')}
            </span>
          </div>
        ))}
      </div>
